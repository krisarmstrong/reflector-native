...
void test_stats_init(void)
{
	...
	if (stats.packets_received != 0 || stats.packets_reflected != 0 || stats.bytes_received != 0 ||
...
void test_config_get(void)
{
	...
	if (config.ifindex != rctx.config.ifindex || config.frame_size != rctx.config.frame_size ||
