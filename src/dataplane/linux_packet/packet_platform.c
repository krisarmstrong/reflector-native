...
int packet_platform_init(reflector_ctx_t *rctx, worker_ctx_t *wctx)
{
    ...
    int qdisc_bypass = 1;
    if (setsockopt(pctx->sock_fd, SOL_PACKET, PACKET_QDISC_BYPASS, &qdisc_bypass, sizeof(qdisc_bypass)) < 0) {
    ...
    if (rctx->num_workers > 1) {
        ...
        if (setsockopt(pctx->sock_fd, SOL_PACKET, PACKET_FANOUT, &fanout_arg, sizeof(fanout_arg)) < 0) {
...
int packet_platform_recv_batch(worker_ctx_t *wctx, packet_t *pkts, int max_pkts)
{
    ...
    for (int i = 0; i < max_pkts; i++) {
        struct tpacket3_hdr *hdr = (struct tpacket3_hdr *)(
            pctx->rx_ring + (pctx->rx_frame_idx * pctx->frame_size));
...
void packet_platform_release_batch(worker_ctx_t *wctx, packet_t *pkts, int num_pkts)
{
    ...
    for (int i = 0; i < num_pkts; i++) {
        ...
        struct tpacket3_hdr *hdr = (struct tpacket3_hdr *)(
            pctx->rx_ring + (frame_idx * pctx->frame_size));
