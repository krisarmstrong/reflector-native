...
static int set_bpf_filter(int fd, const uint8_t mac[6])
{
	...
	struct bpf_program filter = {.bf_len = sizeof(insns) / sizeof(insns[0]), .bf_insns = insns};
...
int bpf_platform_init(reflector_ctx_t *rctx, worker_ctx_t *wctx)
{
	...
	if (ioctl(pctx->bpf_fd, BIOCSETIF, &ifr) < 0) {
		int saved_errno = errno;
		reflector_log(LOG_ERROR, "Failed to bind BPF to %s: %s", wctx->config->ifname,
		              strerror(saved_errno));
		close(pctx->bpf_fd);
		free(pctx);
		errno = saved_errno;
	...
	if (ioctl(pctx->write_fd, BIOCSETIF, &ifr) < 0) {
		int saved_errno = errno;
		reflector_log(LOG_ERROR, "Failed to bind write BPF to %s: %s", wctx->config->ifname,
		              strerror(saved_errno));
		close(pctx->bpf_fd);
		close(pctx->write_fd);
		free(pctx);
	...
	reflector_log(
	    LOG_INFO,
	    "BPF platform initialized on %s (buffer: %zu KB, kqueue: enabled, batching: enabled)",
